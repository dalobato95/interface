...
Important note for users of versions prior to 1.2:
- The behavior of the earlier version of this addon was to allow dragging it anywhere on the screen. In the spirit of keeping it even closer to the original default, the default behavior is now to dock on the left side of the screen and move over like default UI panels do.
- If you want you can still drag the window around the screen by checking 'Undock Window' in options.
