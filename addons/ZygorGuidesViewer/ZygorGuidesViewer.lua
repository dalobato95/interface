...
function ZGV:SkipStep(fast,hack,forcefocus) --Hack used for testing.
	...
	if not nextstep then  -- when not forced, that is: usually.
	...
	if not nextstep --[[or (self.CurrentStep and (self.CurrentStep.num == #self.CurrentGuide.steps))--]] then
		-- final step
		self.pause = true
		self.fastforward = false
		if self.CurrentGuide.next and ZGV.db.profile.n_popup_guides then
	...
	if self.CurrentGuide and self.CurrentGuide.steps and self.CurrentGuide.fully_parsed then
		...
		for stepframenum = 1,self.StepLimit do repeat
			...
			if (stepdata:AreRequirementsMet() or self.db.profile.showwrongsteps) then
				...
				if not self.db.profile.showinlinetravel then for i,goal in ipairs(goals) do if not goal:IsInlineTravel() then canhidetravel=true break end end end
